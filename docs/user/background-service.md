...
On Linux and macOS, PseudoCode can run as a service for your user so you do not need
to keep a terminal open.

## Manage the service

Run these commands on the machine that will host PseudoCode:
...
[Updating PseudoCode](./updating.md).

## Platform support

Linux needs systemd user services. Setup enables lingering so PseudoCode starts at
...
with sudo; running PseudoCode as root creates a separate installation and Connect
...
| `service-disabled` or `service-stopped` | Read the log and `systemctl --user status t3code.service`, then use the repair command printed by PseudoCode.                  |
