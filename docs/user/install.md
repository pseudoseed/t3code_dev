...
PseudoCode is a web and desktop GUI for running coding agents on your machine.

## Requirements

Node.js `^22.16 || ^23.11 || >=24.10` on the machine that runs the PseudoCode server.

At least one provider runtime, installed and authenticated. You can install Antigravity from
PseudoCode settings. See [Providers](#providers) below.
...
This starts the PseudoCode server on your machine and opens the local web app. Use
`npx t3@latest --help` for the full CLI reference.

If the web or desktop app shows "PseudoCode could not load", check your connection and select
**Reload** to try again.

## Open a project in the desktop app

When the PseudoCode desktop app is running on the same machine, open the current directory with:
...
thread. It does not launch the desktop app, open a browser, or start a PseudoCode server. A background
...
successful launch, PseudoCode keeps the current runtime and one previous runtime for rollback and
removes older caches automatically. If a cached runtime stops working, PseudoCode launches from the
application files under `/mnt/c` instead and reinstalls the runtime on the next launch.

## Providers

PseudoCode uses provider runtimes but does not bundle them. Install and authenticate each
provider's CLI, or use PseudoCode's managed setup for Antigravity.
...
| Antigravity | [Official ACP agent](https://github.com/agentclientprotocol/registry/blob/main/antigravity-acp/agent.json) | Managed by PseudoCode | **Sign in with Google** in PseudoCode |
...
PseudoCode looks for, but authenticate with `agent login`, not `cursor-agent login`.
...
Run CLI login commands on the machine running the PseudoCode server, not on the device you browse
from. Antigravity uses its sign-in controls in PseudoCode instead of a CLI login command.
...
PseudoCode. You can install PseudoCode, open it, and add providers afterwards. A provider that is not
...
- [Permission modes](./permission-modes.md): how much PseudoCode asks before acting
- [Remote access](./remote-access.md): connect from a phone, tablet, or another desktop
- [Keeping PseudoCode in sync](./updating.md): client and server version skew
