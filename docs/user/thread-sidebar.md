...
live background work keep the thread active. PseudoCode settles from a closed or merged pull request
...
If reordering is unavailable for one environment, update the PseudoCode server running in that
...
PseudoCode keeps track of this on the server, so reading a thread on your desktop clears it on your
...
If one environment does not show the mark, update the PseudoCode server running there. Older servers
...
Your choice is remembered and comes back the next time you open PseudoCode. A project you add later
...
pill** fallback because their colors are not controlled by PseudoCode.

To generate a fresh title from the conversation, open a thread's context menu and choose
**Regenerate title**. While PseudoCode is generating it, the action reads **Regenerating…** and cannot
