...
**SSH**, and enter a host or SSH alias such as `user@example.com`. PseudoCode starts
...
the connection stops a server that PseudoCode launched; a server that was already
...
| Error                                                     | Recovery                                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart PseudoCode on the host.                                                                      |
...
| Expired or invalid link proof                             | Check the host's date and time, update PseudoCode, then restart it.                                                                         |
...
devices. For server version warnings, follow [Updating PseudoCode](./updating.md).
...
environment**. PseudoCode restarts without a local server: no local agents or terminals run, WSL
backends stay off, and other devices can no longer connect to this computer. Your projects,
history, and saved connections are kept, and you keep working through pairing, Cloud Connect, or SSH.
