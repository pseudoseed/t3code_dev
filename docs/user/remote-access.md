...
Use this when you want to connect to a PseudoCode server from another device such as a phone, tablet, or separate desktop app.
...
run a headless server from the CLI, or have the desktop app launch PseudoCode over SSH.
...
Use this when you want the desktop app to start or reuse PseudoCode on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. PseudoCode uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, PseudoCode first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path PseudoCode uses:
...
Copy that full address into the return URL field in the same PseudoCode client. Choose
...
When the PseudoCode web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: PseudoCode may
...
If Cloud Connect cannot connect, check the date and time on both devices, then try again.

Finish active work before updating because the server restarts briefly. For step-by-step guidance,
see [Keeping PseudoCode in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running PseudoCode in the Background](./background-service.md).
...
Hosted pairing does not proxy traffic through PseudoCode. The browser still connects directly to the backend URL in the pairing link.
...
### Deregister a Cloud Connect Environment

Open your account menu and choose **Cloud Connect** to see every environment registered to your
account. On mobile, open **Settings** → **Cloud Connect**. Choose **Deregister** to revoke an
environment's Cloud Connect access, remove any managed tunnel, and free its host space.
