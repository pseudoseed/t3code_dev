...
PseudoCode runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for PseudoCode to confirm
...
hostname. Only that PseudoCode sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. PseudoCode's separate Plan mode
...
PseudoCode keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. PseudoCode also reads `.gemini/skills` and the
