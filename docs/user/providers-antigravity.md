...
PseudoCode runs Google's official Antigravity ACP agent. By default it signs in with your personal
...
not sign in this PseudoCode provider.
...
2. Choose **Install Antigravity**. PseudoCode downloads the official runtime from Google to that
...
6. Wait for PseudoCode to confirm sign-in and load the model choices. Select an Antigravity model
...
machine as the PseudoCode environment, sign-in can finish directly.
...
return URL field in PseudoCode. Choose **Continue** on web or desktop, or **Complete sign-in** on
mobile. Do not change the address to your server's hostname.

Return to the same PseudoCode client and environment where you started sign-in. Another client
...
a thread or bug report. PseudoCode waits for Google's agent to confirm sign-in. A successful
...
are stored in plain text in PseudoCode settings on that environment and are passed only to the
...
PseudoCode does not download it until you choose to install it.

On web or desktop, **Update Antigravity** appears when PseudoCode has a newer managed release.
...
A nonempty **Binary path** takes priority. With the field empty, PseudoCode uses its managed
runtime, then an installation on the environment's `PATH`. An invalid explicit path reports
an error instead of selecting another installation. PseudoCode does not update or remove manual
installations. Clear **Binary path** to use managed installation controls.

## Models and threads

The current official ACP exposes Gemini models only. PseudoCode uses the model IDs and names
...
PseudoCode asks you to select an available model instead of silently changing it.

Use Antigravity's native `/plan` command to request a plan. PseudoCode's separate Plan mode control
is not available for this provider.

Project skills should use `.agents/skills`. PseudoCode also discovers `.gemini/skills` and the legacy
...
Antigravity reads and edits workspace files through PseudoCode. Each write shows up as a file
...
When the agent offers **Allow for this thread** on a shell or web tool, PseudoCode shows Google's
...
PseudoCode keeps thread history and file diffs. Antigravity does not support conversation rewind,
...
installation. PseudoCode refuses removal while the runtime is in use. An explicit **Binary path**
that points into the managed runtime must be cleared first.

## Account access and errors

Google controls account eligibility, models, and usage limits. PseudoCode does not report your
...
Use any retry time Google supplies. PseudoCode does not switch to an API key to get past the limit.
