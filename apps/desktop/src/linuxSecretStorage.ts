...
function getGnomeKeyringRemediationMessage(): string {
  return "PseudoCode could not access GNOME Keyring to save this environment credential. Install and start GNOME Keyring, then restart PseudoCode.";
}

function getKWalletRemediationMessage(): string {
  return "PseudoCode could not access KWallet to save this environment credential. Enable the KDE wallet subsystem in System Settings, then restart PseudoCode.";
}
