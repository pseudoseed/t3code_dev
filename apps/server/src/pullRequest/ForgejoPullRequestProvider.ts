...
const CAPABILITIES: PullRequestCapabilities = {
  ...
  reactions: true,
  labels: true,
  // Replies are line notes; draft conversion uses Forgejo's WIP title convention.
  review: {
    inlineComment: true,
    reply: true,
    resolve: false,
...
export const make = Effect.gen(function* () {
  ...
  const forkApi = yield* ForgejoPullRequestApi.ForgejoPullRequestApi.pipe(
    Effect.provide(ForgejoPullRequestApi.layer.pipe(Layer.provide(ForgejoTransport.layer))),
  );
  const forkFailure = (error: ForgejoPullRequestApi.ForgejoPullRequestApiError) =>
          ...
          new PullRequestProviderError({
            ...
            reason:
              error.reason === "missing-cli"
                ? "missing-tool"
                : error.reason === "authentication" || error.reason === "forbidden"
                  ? "unauthenticated"
    ...
    return {
      actions: active
        ? CAPABILITIES.actions.filter((action) =>
            ["merge", "update-branch", "enable-auto-merge", "disable-auto-merge"].includes(action)
              ? canWrite
              : canEdit,
          )
        : [],
  ...
  const provider: PullRequestProviderApi = {
      ...
      function* (input) {
        ...
        for (let index = firstIndex; more && consumed < input.limit; index++) {
          ...
          for (const row of rows.slice(start)) {
            ...
            consumed++;
            if (row) {
              const item = forgejoChangeRequest(row);
              const viewer = input.viewer.toLowerCase();
              const needle = input.query?.trim().toLowerCase() ?? "";
              if (input.state === "merged" && !row.merged) continue;
              if (input.state === "closed" && row.merged) continue;
              if (input.involvement === "authored" && row.user?.login.toLowerCase() !== viewer)
                continue;
              if (
    ...
    getChangeRequestActivity: Effect.fn("ForgejoPullRequestProvider.getChangeRequestActivity")(
      function* (input) {
        ...
        return {
          ...
          reviewThreads: [
            ...inline
              .reduce((threads, comment) => {
                const thread = forgejoReviewThread(comment);
                const id = ForgejoPullRequestApi.encodeThreadId({
                  path: thread.path,
                  position: thread.line,
                });
                const entry = byId.get(String(comment.id)) ?? forgejoComment(comment);
                ...
                return threads;
              }, new Map<string, ReturnType<typeof forgejoReviewThread>>())
              .values(),
          ],
    ...
    replyToThread: (input) => forkApi.replyToThread(input).pipe(Effect.mapError(forkFailure)),
