...
export const make = Effect.fn("desktopUpdate.desktopAppUpdate.make")(function* () {
    ...
    Effect.gen(function* () {
      ...
      if (report.outcome === "up-to-date") {
        return yield* failWith(
          `The PseudoCode desktop app on this machine is already up to date on ${report.state.currentVersion}.`,
        );
      }
      return yield* failWith(
  ...
  const run: DesktopAppUpdate["Service"]["run"] = Effect.fn("desktopUpdate.desktopAppUpdate.run")(
    function* (reportProgress) {
      if (!available) {
        return yield* failWith(
          "This server was not started by the PseudoCode desktop app, so it cannot drive a desktop update.",
        );
      }
      ...
      return yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* receiver
            ...
            .pipe(
              Effect.mapError((error) =>
                failWith("Could not reach the PseudoCode desktop app on this machine.", error),
              ),
            );
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.uninterruptible(
          receiver.commitDesktopUpdate(requestId).pipe(
            Effect.mapError((error) =>
              failWith("Could not reach the PseudoCode desktop app.", error),
            ),
