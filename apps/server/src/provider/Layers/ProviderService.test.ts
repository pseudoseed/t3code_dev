...
describe("agent browser access", () => {
  ...
  it.effect("issues a credential without preview when agent browser access is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["issues", "mailbox", "pull-requests"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "issues", "mailbox", "preview", "pull-requests"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("drops only the preview capability when browser access alone is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "issues", "mailbox", "pull-requests"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when the project overrides browser access to on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["issues", "mailbox", "preview", "pull-requests"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
      ...
      const providerLayer = makeProviderServiceLive({
        issueMcpCredential: (request) =>
          Effect.sync(() => {
            ...
            assert.ok(request.capabilities.has("issues"));
            assert.ok(request.capabilities.has("mailbox"));
            return undefined;
          }),
      ...
      const turns = yield* Effect.gen(function* () {
        ...
        const seedOldCredential = () =>
          McpProviderSession.setMcpProviderSession({
            ...
            capabilities: new Set(["preview", "issues", "mailbox"]),
          });
